...
class Car:
    ...
    def _corners_on_track(self, track: Track) -> bool:
        return all(track.is_on_track(x, y) for x, y in self._car_corners())
    ...
    def sensor_lines(self, track: Track) -> List[Tuple[Tuple[float, float], Tuple[float, float]]]:
        """Geometry helper used by renderers to draw sensor rays."""
        lines = []
        ...
        for ratio, offset in zip(readings, self.sensor_angles):
            ...
            lines.append(((self.x, self.y), end))
        return lines

    def polygon_points(self) -> List[Tuple[float, float]]:
        """Geometry helper used by renderers to draw the car body."""
